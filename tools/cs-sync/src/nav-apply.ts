...
import { articleFileName, EXCLUDED_CHAINS } from "./lib/nav-shared.js";
...
/** Per-product ceiling on Pass 2 deletions. See the comment at that pass. */
const DEFAULT_MAX_DELETIONS = 25;
const parsedMaxDeletions = Number.parseInt(
  process.env.NAV_APPLY_MAX_DELETIONS || String(DEFAULT_MAX_DELETIONS),
  10,
);
const maxDeletions = Number.isFinite(parsedMaxDeletions) ? parsedMaxDeletions : DEFAULT_MAX_DELETIONS;

/**
 * How stale a .nav-tree.json may be before this refuses to act on it.
 *
 * The snapshot is gitignored and regenerated by hand, so it can silently
 * describe a nav from weeks ago while the writes it drives look current. It also
 * bakes in the EXCLUDED_CHAINS in force when it was crawled, which is a subtler
 * trap: the 2026-09-14T12:04Z snapshot was taken while
 * "headless-cms/developer-tools-delivery/cli" was still excluded, so it was
 * missing 94 CLI leaves that the code no longer excludes at all.
 */
const MAX_TREE_AGE_HOURS = Number.parseInt(process.env.NAV_APPLY_MAX_TREE_AGE_HOURS || "6", 10);
...
// The local escapeForFrontmatter that used to live here was a copy of the one in
// lib/entry-to-markdown.ts, and it carried the same bug: it escaped a quote but
// not a backslash already in front of it, so a CMS description holding `\"` came
// out as `\\"` and closed the YAML scalar early. The module header in
// lib/entry-to-markdown.ts says that converter exists so copies cannot drift.
// This file had drifted, so it now imports yamlQuoted/yamlScalar instead.
...
/**
 * Refuse a snapshot this run should not be writing files from.
 *
 * Two failure modes, both seen for real:
 *   - Age. The snapshot is gitignored and hand-regenerated, so it can describe a
 *     nav from weeks ago while looking authoritative. master-design.md records a
 *     33-day-old one being used.
 *   - Drifted filters. A snapshot bakes in the EXCLUDED_CHAINS active when it was
 *     crawled. The 2026-09-14T12:04Z one was taken while the headless-cms CLI
 *     chain was still excluded and was missing 94 leaves; acting on it would
 *     have made those 94 pages look like files the nav no longer wanted.
 */
function assertTreeIsUsable(tree: NavTree): void {
  const ageHours = (Date.now() - Date.parse(tree.generatedAt)) / 3_600_000;
  if (!Number.isFinite(ageHours)) {
    console.error(`Nav tree has an unreadable generatedAt (${tree.generatedAt}). Run: npm run nav-tree`);
    process.exit(1);
  }
  if (ageHours > MAX_TREE_AGE_HOURS) {
    console.error(
      `\n🛑 Nav tree is ${ageHours.toFixed(1)}h old (limit ${MAX_TREE_AGE_HOURS}h), generated ${tree.generatedAt}.\n` +
        `   Run: npm run nav-tree\n` +
        `   Or raise NAV_APPLY_MAX_TREE_AGE_HOURS if you know the nav has not moved.`,
    );
    process.exit(1);
  }
  ...
  if (!tree.provenance) {
    console.error(
      `\n🛑 Nav tree has no provenance stamp, so the filters it was crawled under are unknown.\n` +
        `   Re-crawl with the current code: npm run nav-tree`,
    );
    process.exit(1);
  }
...
export function buildSampleApp(entry: Entry): string | null {
  ...
  const frontmatter = [
    ...
    `url: ${yamlScalar(String(entry.url ?? ""))}`,
    "doc_type: sample-app",
    "---",
  ].join("\n");
  return `${frontmatter}\n\n# ${heading}\n\n${body}\n`;
...
export function buildFaqFiles(
  ...
  sections.forEach((section, sectionIndex) => {
    ...
    faqs.forEach((faq, index) => {
      ...
      const frontmatter = [
        "---",
        `title: ${yamlQuoted(question)}`,
        `description: ${yamlQuoted(question)}`,
...
async function applyProduct(
  ...
  //
  // Capped, because `keep` is built from a live crawl and a fetch that comes
  // back thin makes this pass look like a legitimate cleanup. The exposure is
  // concrete: 1,491 FAQ and troubleshooting files hang off 19 product_faqs_2026
  // container leaves, and they only land in `keep` because buildFaqFiles
  // enumerated their questions. If one container fetch fails, every one of its
  // files becomes a delete candidate. cms-pull-prod.ts has had a cap of 10 for
  // exactly this reason (DEFAULT_MAX_DELETIONS); this pass had none.
  const existing: string[] = [];
  listMarkdown(path.join(repoRoot, DOCS_ROOT, slug), existing);
  // `consumed` holds the files Pass 1 moved to a new nav-derived path. In a real
  // run they are already gone from their old location and never show up in
  // `existing`. Under --dry-run movePath is a no-op, so they are still sitting
  // there and would otherwise be counted as deletions: a pure restructure would
  // report "86 files would be deleted" and trip the cap, which is both alarming
  // and wrong. Excluding them makes the dry run mean what it says.
  const doomed = existing
    .map((abs) => path.relative(repoRoot, abs))
    .filter((rel) => !keep.has(rel) && !consumed.has(rel));

  if (doomed.length > maxDeletions) {
    console.error(
      `\n🛑 ${slug}: ${doomed.length} files would be deleted, over the limit of ${maxDeletions}.\n` +
        `   Refusing, because a thin nav crawl is indistinguishable from a real cleanup here.\n` +
        `   First 10:\n${doomed.slice(0, 10).map((r) => `     - ${r}`).join("\n")}\n` +
        `   If this is genuinely intended, re-run with NAV_APPLY_MAX_DELETIONS=${doomed.length}.`,
    );
    process.exit(1);
  }
