---
title: "FindAsync"
description: "Retrieves all variants for a specific entry asynchronously."
url: "https://www.contentstack.com/dotnet-management-entryvariant-findasync"
...
Returns:
Type
Task<ContentstackResponse>. Resolves to the variants associated with the specified entry.
...
- Sends a GET request to `/content_types/{contentTypeUid}/entries/{entryUid}/variants` asynchronously. Returns the raw API response wrapped in `ContentstackResponse`.
- The SDK does not paginate automatically. Use `collection` to control `limit` and `skip`.
