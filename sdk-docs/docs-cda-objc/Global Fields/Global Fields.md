---
title: "Global Fields"
description: "A Global field is a reusable field (or group of fields) that you can define once and reuse in any content type within your stack. This eliminates the need (and thereby time and efforts) to create the same set of fields repeatedly in multiple content types. Example: Stack *stack = [Contentstack stackWithAPIKey:@\"API_KEY\" accessToken:@\"DELIVERY_TOKEN\" environmentName:@\"ENVIRONMENT\"]; GlobalField *globalfield = [stack globalFieldWithName :@\"Global_field_uid\"]; [globalfield fetch:^(ResponseType type, NSError * _Nullable error) { }];"
