...
- **301 nav leaves point at entries Production does not publish, and the reconcile wrote files for all 301.** Re-measured 2026-09-15, against the 19 this section first recorded. The rise comes from better `article_via_url` detection, which now finds 358 of those leaves against 60 in the older snapshot. 296 of the 301 are Lytics CDP, plus 1 Android SDK article and 4 sample apps. **Most of them are published, to staging and development, but not to Production**, which is exactly why only Production counts. Neither `nav-apply.ts` nor `nav-verify.ts` read the published flag, which is how the files got written. Both read it as of 2026-09-15, and the 296 Lytics files are deleted. The nav positions remain, so those urls are live 404s until someone removes the links. The list sits at `tools/cs-sync/nav-audit/unpublished-nav-leaves.csv`.

  **Check publish state with `include_publish_details=true`.** Without that query parameter the Content Management API (CMA) returns no `publish_details` field for any entry, published or not. A spot check that omits it reports every entry as unpublished, including pages that serve a 200 on the live site. `nav-tree.ts` passes it, so the crawl's `prodPublished` flag is sound. Confirm against the live url as a second signal: a Production-published page returns 200 and an unpublished one returns 404.
