...
5.  Use the **Field Properties** panel, which opens on the right, to configure the field behaviour. The available options vary by field type and can include:
    ...
    -   **Selection Settings**: For Select, Radio, and Checkbox fields, define the choice list, the selection mode, the data type, and the choice format. Refer to [Configure Select Fields](/docs/assets/configure-select-fields) for the full set of options.
...
-   Use a Select, Radio, or Checkbox field wherever the value comes from a known set, such as region or license type. Free-text values drift and stop filtering together.
