...
The Contentstack iOS Persistence SDK (Objective-C) is planned for deprecation. There is no replacement persistence SDK. If you continue to use this SDK, be aware of maintenance and security risks.
...
-   [Contentstack’s iOS SDK](/docs/developers/sdks/content-delivery-sdk/ios/about-objective-c-sdk/)
...
1.  Download and set up the Contentstack iOS Persistence Library from [here](https://github.com/contentstack/contentstack-ios-persistence.git).  
...
To start mapping of data, first, you need to create a content type schema (in Contentstack) as per your app design and create entries. For your convenience, we have already created the necessary content types. Download them from [here](https://drive.google.com/open?id=1jnpNIHRb4kNcP3r0I2QMatGzPwNXxpzS) and [import them](/docs/headless-cms/import-a-content-type) to your app’s stack in Contentstack.

Next, [create entries](/docs/headless-cms/about-entries) for the imported content types. In order to sync this data with Realm, we need to add data mappings. The three important items to be mapped in our Synchronization process are as follows:
