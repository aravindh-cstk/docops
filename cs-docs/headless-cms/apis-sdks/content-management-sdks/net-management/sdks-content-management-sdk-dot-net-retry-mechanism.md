...
![Retry flow decision diagram](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/blt00c4ca7a6b563b85/69956852ca1be7000834cc06/NET_CMA_Retry_Architecture.png)

1.  **Execute Request** → Check response/exception.
2.  **Success (2xx)** → Return response.
3.  **HTTP Error** → Check if retryable → Check limit → Retry or fail.
4.  **Exception** → Classify error type:
    -   **Network Error:** Check network retry config → Check NetworkRetryCount < MaxNetworkRetries → Retry or fail.
...
5.  **Calculate Delay**:
    -   Check Retry-After header (for 429) → Use if present.
...
6.  **Wait** → Loop back to Step 1.
7.  **Failure** → Log error → Throw exception.
...
1.  RetryOnError is true.
2.  Retry limit is not exceeded for the error type.
3.  The error is classified as retryable. By default, the SDK retries only transient failures.
