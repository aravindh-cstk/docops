...
1.  Both **ordered (OL)** and **unordered (UL)** lists are supported in Contentstack’s JSON RTE.
2.  **Tables** are the only block elements allowed to nest within list items.
3.  **List markers** shown in the RTE are for visualization only and are not delivered with the content.
4.  Frontend developers must apply **CSS styling** to ensure consistent display across different platforms.
