...
**Note:** Custom Preview URLs is available on select plans. Contact Contentstack [support](mailto:support@contentstack.com) for more information.
...
```
<environment base URL> + <entry's native URL field>
```
...
1.  **Pattern selection:** Selects the matching configuration based on branch and content type.
2.  **Pattern resolution:** Replaces placeholders in the pattern using entry data, environment context, locale, and taxonomy values.
3.  **Preview rendering:** Loads the resolved URL in the Live Preview iframe.
4.  **Fallback behavior:** Reverts to the default preview URL if resolution fails.

## Custom Preview URLs in Visual Editor

Custom Preview URLs resolve in Visual Editor the same way they do in Live Preview. After you configure a pattern under **Settings** → **Visual Experience** → **Preview URL**, Visual Editor uses that configuration to resolve entry URLs on the canvas. No additional setup is required.

To resolve a custom URL, Visual Editor follows the same phases as Live Preview: 

-   Selects the matching configuration for the branch and content type
-   Replaces the placeholders with entry, environment, locale, and taxonomy values
-   Loads the resolved URL on the canvas. 

If any required value is missing, Visual Editor displays the unresolved fields so you can complete the entry before previewing it.

When an entry relies on a custom URL pattern, its preview URL is generated from the pattern instead of a single URL field. As a result, Visual Editor cannot identify the entry from the target URL alone. 

To open the correct entry, the **Start editing** button passes the entry UID and content type UID as page context. Configure this context once in your front-end code.

For setup steps, refer to [setPageContext(context)](/docs/headless-cms/get-started-with-live-preview-utils-sdk-v4#setpagecontextcontext) in the Live Preview Utils SDK config reference.

**Note:** For internal navigation, such as opening an entry from the entry list or the Tasks page, no extra setup is required, because Contentstack already passes the entry UID and content type UID in the URL.

### Locale Selection in Visual Editor

In Visual Editor, the locale selector lists all locales available for your custom URL configuration, not just the locales mapped to a base URL in your environment settings. Select a locale to preview and edit the entry in that language.
...
1.  Go to your [stack](/docs/headless-cms/about-stack) and click “Settings”.
...
6.  Build **preview URLs** using dynamic placeholders in the pattern. Placeholders use the {{...}} syntax and can reference:
    ...
    2.  **Custom fields**: {{entry.custom\_field\_id}}
...
1.  Select a content type, environment, and locale.
2.  Choose a base URL alias.
3.  Preview the resolved URL.

![4._Test_Your_Configuration.png](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/bltb172b3fb2ec38ebc/69bd7d555af52351ae96028c/4._Test_Your_Configuration.png)
...
![5._Advanced_Configuration.png](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/bltcf186adc4d21277b/69bd7d6cd4ee4e85acd09fe2/5._Advanced_Configuration.png)
...
1.  **Text case transformation**: Convert values to a consistent format.
    1.  lowercase: Technology → technology
    2.  uppercase: blog → BLOG
2.  **Array handling**: Process multi-value fields such as taxonomies or references.
    1.  join values → \["Technology", "Cloud Computing"\] → technology/cloud-computing
3.  **Value mapping**: Map values to custom outputs (useful for locales, environments, or categories).
    1.  "en-us" → "en"
    2.  "Artificial Intelligence" → "ai"
4.  **Regex transformations**: Modify values using pattern-based replacements.
    1.  Replace spaces with hyphens: "hello world" → "hello-world"
    2.  Remove special characters: "AI & ML!" → "ai-ml"
    3.  Normalize accented characters: "Crème Brûlée" → "creme-brulee"
5.  **Default values:** Provide fallback values when fields are missing or empty.
    1.  null → "uncategorized"
6.  **Chained transformations:** Apply multiple operations sequentially to achieve the desired output.
    1.  "Blog Article" → "blog article" → "blog-article"
...
1.  **Pattern Resolution**: The system attempts to resolve all placeholders using entry data, locale, environment, and taxonomy values.
