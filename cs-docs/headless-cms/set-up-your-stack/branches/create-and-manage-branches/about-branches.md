...
When you create a branch for the first time, the main branch becomes your source branch. For all subsequent branches you create, specify a source branch from which it inherits data.
...
Any child branch inherits all data from the source branch, including content types, global fields, entries, assets, languages, webhooks, and extensions. Now, when you make a change in the child branch, it does not affect the parent branch.

When you create a branch, you can also choose which environments to clone into it — all environments, none, or a specific set. All other data, such as content types, entries, assets, and languages are inherited from the source branch as usual. For more information, refer to the [Create a Branch](/docs/headless-cms/create-a-branch) documentation.
