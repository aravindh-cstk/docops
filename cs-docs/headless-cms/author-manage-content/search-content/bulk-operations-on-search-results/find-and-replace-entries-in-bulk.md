...
description: "Find and Replace Entries in Bulk"
...
5.  In **Replace With**, enter the replacement text. To remove the matching text instead, select **Clear**.
...
2.  Review the matches under **Preview Matching Results**. Each row shows the **Entry**, **Locale**, **Field**, and **Updates**; select **View Updates** to see the before-and-after for an entry.
3.  Select or clear individual changes or entries to control exactly what applies. The preview reflects matches when you run the find; the replacement applies to the latest version, so results can differ if entries change in between.
4.  Click **Apply Selected Changes**.
5.  In the **Replace Text** dialog, review the summary (for example, "Replace text in 100 fields across 32 entries"), optionally add the entries to a release (see below), then click **Replace Text**.
6.  Track progress in the **Bulk Tasks** panel, it shows **Tasks in Progress** while the job runs and **Tasks Completed** when it finishes. Open it to review **All Entries**, **Successful Entries**, and **Failed Entries**, and select **Retry** on any failed entry.
...
-   One find operation allows one replace operation. Reopening a job from the **Bulk Tasks** queue restores it in the exact state it had when you ran the find, you can review the results, but you can't regenerate the preview or replace it again. To run another replacement, select **Clear** to reset the **Find** and **Replace With** fields, then start a new find.
