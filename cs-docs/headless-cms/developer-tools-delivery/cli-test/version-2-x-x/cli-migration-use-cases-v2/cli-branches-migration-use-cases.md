---
title: "Branches | Migration Use Cases | V2.x.x"
description: "Branches | Migration Use Cases"
url: /headless-cms/cli-branches-migration-use-cases
uid: blt6702caeefc36568f
---

# Branches | Migration Use Cases | V2.x.x
...
**Note:** We assume you are well versed with the migration script and migration plugin. If not, read our detailed documentation on how to [get started with the migration script](/docs/headless-cms/migrate-content-between-stacks-using-the-cli#get-started-with-the-migration-script) and [use the migration plugin](/docs/headless-cms/migrate-content-between-stacks-using-the-cli) to migrate your content.
...
5.  Use the migration plugin command given below to apply your changes to the main branch. If the management token is not specified, by default the migration plugin uses Auth token.

    ```
    csdx cm:stacks:migration --file-path <file_path> -k <api_key> --branch <branch_name>
    ```
...
A branch merge runs as a background job, so a migration that merges branches does not have to block while it finishes. Use cm:branches:merge-status with the merge job UID returned when the merge started to check whether it has completed before moving on to the next step.
