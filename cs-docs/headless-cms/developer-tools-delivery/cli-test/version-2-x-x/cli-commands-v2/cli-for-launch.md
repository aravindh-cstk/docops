...
-   [CLI installed](/docs/headless-cms/install-the-cli/v1) and [configured](/docs/headless-cms/configure-regions-in-the-cli) for AWS (version 1.6.0 and above), Azure (version 1.23.0 and above), or GCP (version 1.31.0 and above).
...
## Install the Launch Plugin

The Launch plugin is not part of the default CLI installation. Install it before running any launch command:

```
csdx plugins:install @contentstack/cli-launch
```

**Warning:** Running a launch command without installing the plugin first exits with code 127 and a command-not-found error. In a CI pipeline, add the install step before any launch command so the run does not fail at that point.
