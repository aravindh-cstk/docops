...
Contentstack comes with a [content delivery network (CDN)](/docs/headless-cms/what-is-cdn-and-how-it-works) that caches data in hundreds of PoP servers around the world. So, blazing-fast content delivery comes by default. And with an adequate purging mechanism in place, your users are sure to get fresh and updated content, always.
...
In your Contentstack account, [set up webhooks](/docs/headless-cms/about-webhooks) for the publish, unpublish or delete events for all [entries](/docs/headless-cms/about-entries) and [assets](/docs/headless-cms/about-assets). This will help you send real-time updates on these events to your caching service. When webhook events are triggered, it brings the relevant data payload, and notifies the destination URL. So, you can accordingly purge the updated or new content from your cache.
