...
To bulk publish entries, sign in to your [Contentstack account](https://www.contentstack.com/login/) and perform the following steps:
...
4. In the **Publish Entries** modal:
 ...
 1. **Select Environment(s)**: Select the environment(s) where the entries are published.
 2. **Select Language(s)**: Select the locale(s) in which you want the entries published. If localized content exists, it is selected by default.
...
- When publishing multiple entries, only the latest version of each entry is published.
- Nested references are limited to a depth of **5 levels**, and only the latest versions of nested references are published.
