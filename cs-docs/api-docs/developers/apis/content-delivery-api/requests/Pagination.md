---
title: "Pagination"
description: GET /content_types/{content_type_uid}/entries?locale={locale}&include_count={boolean_value}&skip={skip_value}&limit={limit_value}
url: developers/apis/content-delivery-api/requests/pagination
...
In both requests, first, use the include_count parameter to get the total count of the items (entries/assets). Learn more about the [Count](#count) parameter.

Since only 100 items are returned at a time in your response (in case of both requests), you can get the rest of the items in batches using the skip parameter in subsequent requests. Learn more about the [Skip](#skip) parameter.
