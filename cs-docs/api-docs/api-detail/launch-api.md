...
**For Launch APIs, we recommend using the**[**Contentstack OAuth**](../../cs-docs/developers/developer-hub/contentstack-oauth.md) **tokens.**
...
For SSO-enabled organizations, the "[Log in to your account](./content-management-api.md#logging-in-out)" request will not return the user authtoken for users who access the organization through Identity Provider login credentials. Consequently, any requests that require user authtoken will not work. Only the owner of the organization and users with permission to access the organization without SSO can use the Launch APIs. Learn more about [REST API Usage](../../cs-docs/developers/single-sign-on/rest-api-usage.md).
...
**Note:** Only the organization [Owners](../../cs-docs/developers/organization/organization-roles.md#organization-owner) and [Admins](../../cs-docs/developers/organization/organization-roles.md#organization-admin) can create OAuth Tokens.
