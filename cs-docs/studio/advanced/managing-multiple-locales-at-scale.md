...
3.  **Project default Language** — what you set in configure-studio.
...
-   **Per-locale caching.** Different routing-layer URLs (path / subdomain / cookie + cache key) already produce separate CDN cache entries — locale-aware caching works without involving Studio.
...
-   **Avoids the editor↔visitor split.** Studio's editor preview already picks locale via its navbar (Layer 1 above); having a separate visitor mechanism that doesn't encode locale into the Studio pattern keeps preview and visitor render aligned.
...
-   Configure Studio — set the project default Language
