...
2.  **Visitor site (you set this).** Your route passes variantAlias explicitly to the SDK as a query _option_ — the **second argument** (CompositionQueryOptions), never a field of the query object and not a studioSdk.init() key:

3.  SSR: csStudio.fetchCompositionData({ url }, { variantAlias: "holiday-2026" })
4.  CSR: useCompositionData({ url }, { variantAlias: "holiday-2026" })
5.  variantAlias is NOT a studioSdk.init(...) config field — the SDK's UserConfig only accepts stackSdk, contentTypeUid, locale, cslp, openInStudioButton, fetchComposition, fetchTemplateEntry. Pass variantAlias per fetch, not on init.
...
-   Configure CSR vs SSR — variant resolution differs between paths
