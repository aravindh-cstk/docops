...
![gf_featured_card Global Field schema: heading, description, image, cta_url.](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/am70beed4ec2760902/39d910aa4ac02cb1f316088a/recipes-multi-schema-gf-featured-card.png)

### Three content types, each embedding the Global Field under a different local name

![Three content types (blog_post, product, event) each embedding gf_featured_card under a different local field name.](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/ameddb9d3b5c515e41/4f14c48796dd301a437965bf/recipes-multi-schema-three-content-types.png)
...
1.  In **Compositions**, open the **Sections tab** and select **\+ New Section**. Name it featured\_card
2.  In **Link Schema**, pick **gf\_featured\_card**
...
3.  On the canvas, drop a **Box** at the root (the card frame) and inside it:

    -   An **Image** component, bind src to template.image
    -   A **Heading**, bind text to template.heading
    -   A **Text**, bind to template.description
    -   A **Button**, bind href to template.cta\_url, label can be a static "See product details" or another binding
4.  **Save**.
...
1.  In **Compositions**, open the **Templates tab** and select **\+ New Template**. Pick blog\_post
2.  On the canvas, drop a featured\_card section

Studio auto-binds: featured\_card linked to gf\_featured\_card, and blog\_post has exactly one gf\_featured\_card field (hero\_card). **A single match, so Studio picks it automatically.**
...
1.  Select **\+ New Template**, then pick product
...
1.  Select **\+ New Template**, then pick event
...
-   Shows the **Data root** in the section's right-panel **Settings**, where the author can switch this instance to the other matching field.
...
![Section selected on a template canvas: right-panel Settings shows the Data root the section is reading from, which the author can switch to another matching field.](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/am1dd710418fc692c5/62f6f6d243bcf621f557df4a/section-data-source-dropdown.png)
...
| --- | --- |
| hero\_card (blog\_post) | Same data\_type (global\_field) + same child shape (heading, description, image, cta\_url), so it matches |
| promo\_card (product) | Same, so it matches |
| banner (event) | Same, so it matches |
| title (blog\_post) | Different data\_type, so Studio skips it |
| start\_date (event) | Different data\_type, so Studio skips it |

Because the field uses a **Global Field**, the shape is literally identical across the three sites. Studio's positional UID remap doesn't even kick in, as it doesn't need to. This is the strongest auto-bind path.
...
![Two content types with Group fields containing children of the same types in the same order but different names. Studio](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/amd32b1f1638c86cf4/7508146e102af4d1de60132f/recipes-multi-schema-groups-remap.png)
