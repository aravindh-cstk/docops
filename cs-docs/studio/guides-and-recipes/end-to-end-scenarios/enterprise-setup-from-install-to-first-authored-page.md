...
> **Before you start: [check that Studio is enabled for your organisation](/docs/studio/check-studio-access).** 10-second app-switcher check. None of the steps below produce a working canvas if Studio isn't turned on.

This recipe assumes:

-   You have a Contentstack stack with at least one content type (e.g. blog\_post) with a few entries
...
Detail: [Section preview route](/docs/studio/section-preview-route), [Template preview routes](/docs/studio/template-preview-routes), [CSR vs. SSR](https://studio-documentation.contentstackapps.com/prompts/configure-csr-vs-ssr.html) for SSR / RSC variants.

## Step 4: Create the Studio Project

In Studio:

1.  Select **\+ New Project**, name it (e.g. "Marketing Site"), and connect it to your stack
2.  In **Settings**, open **General** and note the Project ID
3.  In **Settings**, open **Configuration**:
...
6.  In the right panel, open the **Data** tab, click the binding chip next to headline, and bind it to template.title
7.  Do the same for cover, binding it to template.featured\_image (or whatever your CT calls it)
8.  Drop a **Card** below the Hero, then bind its title to template.title, body to template.body, and image to template.featured\_image
9.  **Save**
...
-   Repeat the template flow for other content types (product, author, and so on)
-   Build reusable sections (e.g. footer, CTA strip). See [Card grid with slots](/docs/studio/card-grid-with-slots)
-   Add per-template-instance overrides for hero copy. See [Overrides without forking](/docs/studio/per-page-component-overrides-without-forking)
...
-   [Templates](/docs/studio/templates-overview)
