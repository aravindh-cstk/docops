...
> **If you already know Contentstack's Modular Blocks field type,** this page is about rendering it via Studio's Smart Containers. The field itself is defined on the CT in Contentstack ([Modular Blocks docs on contentstack.com](https://www.contentstack.com/docs/developers/create-content-types/modular-blocks)). Studio doesn't change what the field is, only how you compose a UI that iterates over it. Studio's Repeater is the visual equivalent of the .map(...) you'd otherwise write manually. The Condition Block is the switch(block.type) inside that map.
...
![Canonical pattern: Repeater bound to blog_post.body_blocks with one Condition Block per block type (hero, feature_grid, quote, cta)](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/ambbf769c886002a36/a2e42b42fd60d6715dbec5c2/containers-modular-blocks-canonical-tree.png)
...
![blog_post content type schema: title, slug, and a body_blocks modular_blocks field with four block-type schemas](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/am2ecf56ba9c63ac9a/b60265a56ad456cc6013f1cc/containers-modular-blocks-schema.png)

A blog post template with this Modular Block authored on the canvas looks like this in the Layers tab:

![Blog Post Layers tree: Container holds a Repeater iterating body_blocks with four Condition Blocks (hero, feature_grid, quote, cta) and their components](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/ama68fd94f1a062506/118e3b9a8e3a8c3a85a61ad5/containers-modular-blocks-layers-tree.png)
...
Internally, this narrowing is what getFieldSchemaAtPath does when it descends into a Modular Block field: with no block uid in scope it returns a **merged** schema (the union of all block fields, used by Studio for auto-binding heuristics). With a block uid in scope, exactly what a Condition Block establishes, it returns that block's schema.

> See composable-studio/src/utilities/sectionCompositionAutoBinding.ts (lines ~60 to 100, getFieldSchemaAtPath + the "blocks" branch) for the source of truth. The same file's doFieldStructuresMatch (line ~197) explains why Modular Blocks are matched **loosely** during section auto-binding: a section's Repeater is typically wrapped in a single Condition Block, so it only needs ONE common block uid with the page's modular block field, not strict superset/subset equality.
...
3.  **Drop the first Condition Block.** Inside the Repeater (use Layers to land it there cleanly), drop a Condition Block. Right panel, Properties, set the condition to When block is hero. The picker scope is now narrowed to the hero block's schema.
...
**Trying to compare or sort across block types.** A Condition Block fans the design out per block type: there's no single binding context that exposes fields from two different blocks side-by-side. If you need cross-block logic (e.g. "show a divider between any two adjacent CTAs"), that belongs in component code, not in Studio's binding graph.
...
A block type's schema can itself contain a Modular Block field: a common shape for recursive content like nested sections, accordion groups, or tabbed layouts. The pattern doesn't change. It nests.
...
![Nested Modular Blocks: a section_group Condition Block contains a Heading and an inner Repeater on repeater.items with text_card and image_card branches](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/am9f24516bd100a024/1926e27bf0da9163ccc0fca9/containers-modular-blocks-nested-tree.png)
...
-   [Repeaters](/docs/studio/create-repeatable-content-with-repeaters): the iteration primitive
