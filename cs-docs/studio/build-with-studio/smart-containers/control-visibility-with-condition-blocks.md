...
A **Condition Block** lets you switch designs based on the current item's type. It is used **inside a [Repeater](/docs/studio/create-repeatable-content-with-repeaters)** when the items the Repeater iterates are **heterogeneous**:
...
| You have | Use a Condition Block? |
| --- | --- |
...
| A homogeneous list where every item has the same shape | No: put a single component inside the Repeater |
| Hide/show based on a non-discriminator boolean | No: use a regular component's visibility prop instead |

If the data is "different things in the same list, each rendered its own way", use a Condition Block. If the data is "same shape every time", a component inside the Repeater is enough.
...
![Condition Block selected via the Layers tab in a Card Grid section: Layers shows Header / Box / Repeater (No source configured) / Section Slot / Condition Block (selected, "No condition configured") / Contents. The Condition Block chip overlaps the section's heading on the canvas because the Condition Block has no DOM box of its own. The right panel shows Configuration, holding When (Select Condition dropdown) and Preview Mode, then Properties, holding Contents.](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/am469476d7bc940edb/3067150a0fbdd2e1d11cd521/condition-block-properties.png)
...
![Page template with a Repeater bound to template.body_blocks and four Condition Blocks per Modular Block type](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/am3d69578904f2ad22/7031b5c39d704bb3aaf7a0d6/containers-condition-modular-blocks-tree.png)

At render time, the Repeater iterates body\_blocks. For each block:

-   A hero block matches the first Condition Block, which renders the Hero.
-   A feature\_grid block matches the second, which renders the feature grid.
...
![Repeater bound to template.recommendations with three Condition Blocks (product, event, article) each rendering its own card](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/am33aae465e58e7d71/87c17ee129069e3b7f216eaf/containers-condition-references-tree.png)
