...
| **Section** | A reusable block dropped inside a template | None (previews on the project's [canvas URL](/docs/studio/canvas-url)) | Optional, via a linked schema |

The flavor chosen at creation time controls where the composition can be used, what URL it gets, and what data it can bind to.

> **Three-part structure.** Any CMS-driven page does three jobs, and Studio's primitives map one-to-one:
> 
> 1.  **Fetch the data** maps to the **Template** (URL-bound, tied to a Content Type). Plus optional pinned entries or saved queries declared on the composition.
> 2.  **Iterate over the schema** maps to a **List Section**, a Section whose root is a Repeater. Used whenever a field holds N items (reference field, modular blocks field, or group with multiple: true).
> 3.  **Bind fields to component props** maps to a **Simple Section**, a Section with no root Repeater. Maps one schema field to one component prop.
> 
...
| [Save vs Deploy](/docs/studio/save-vs-deploy-a-composition) | The cycle from Save to Deploy to published |
