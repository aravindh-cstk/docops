...
![Two side-by-side cards comparing a hard-coded prop and a bound prop. The Hard-coded card shows { title:](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/amb71f798c7a9f931f/7e47338b9041a93dda255ce4/composition-hard-coded-vs-bound.png)
...
2.  **data-cslp tag emission**, the second job the binding map does at render time: emitting the data-cslp tags Visual Editor consumes. See [data-cslp tag emission](#tag-emission-visual-editor-consumes-what-studio-emits), orthogonal to authoring. Skip on first read if VE isn't installed.
...
| Where you pointed the prop | What the visitor's browser renders |
| --- | --- |
...
1.  **A direct input**: type a value, select from a dropdown, select a date, etc.
...
| **Component Default Data** (any context, when a component is selected) | Whatever you pass via the data prop on <StudioComponent specOptions={...} data={...} />. This is the canonical hook for **bringing external data into a composition** (live pricing, geo, feature flags, weather, anything outside Contentstack). See [Component Default Data: runtime data injection](/docs/studio/set-component-default-data#part-2-runtime-component-default-data-the-prop-on). |
...
![Layers tree: Page (blog_post) contains a Repeater bound to template.related_posts, with Heading and Text inside bound to repeater.title and repeater.excerpt](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/am47ebac1ff33b191b/245d446568bbeec917a9f8cc/composition-binding-inside-repeater-tree.png)
...
![Layers tree: Repeater bound to template.body_blocks. Condition Block matches when block.type equals hero. Heading inside binds to template.headline](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/amae0ee52026a7004f/30b7e4abefe6b46c9f5658eb/composition-binding-condition-block-tree.png)
...
The most valuable use of binding: one Connected template renders every entry of the connected content type. Bindings inside the template resolve against the current entry. The entry changes per URL.

![Connected blog_post template wired to /blog/{{entry.url}} with Heading, Image, Text, and a Repeater of Cards bound to template fields](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/am01547465bd5b2778/543f5338f7090419d98236c3/composition-connected-template-tree.png)
...
The bindings stay the same. The data behind them switches. See [Install the Studio SDK: Fetch a composition](/docs/studio/install-the-studio-sdk#4-fetch-a-composition-with) for the locale / variantAlias options.
...
| Button size variant ("large", "medium") | Yes | unless varies per content |
...
**Bind early, customise later.** Drop a component, bind every prop that's content, and only then start tweaking styles. This builds in the data dependency from the start.
...
-   [Install the Studio SDK: Fetch options](/docs/studio/install-the-studio-sdk#4-fetch-a-composition-with): locale, variantAlias, etc.
