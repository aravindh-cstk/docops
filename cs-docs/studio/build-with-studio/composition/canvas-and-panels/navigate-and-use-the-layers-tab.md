...
Slots show up as their own rows with their label, and filled slot contents nest underneath. You can verify the chained section-inside-slot pattern is wired correctly by reading the tree.
...
Default layer names are the component's display name (Box, Heading, Section: card\_grid). Useful for identifying what a layer is, less useful for identifying what it means in the composition.

Right-click a layer: **Rename**. Give it a meaningful name:

-   Box becomes Hero container
-   Heading becomes Page title
-   Box becomes Footer wrapper

Names are per-composition: they don't change the component everywhere, only this drop instance.
