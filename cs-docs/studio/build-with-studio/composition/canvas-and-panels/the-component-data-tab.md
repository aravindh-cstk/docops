---
title: "The Data tab in the right panel"
...
-   If nothing is selected, the panel shows the empty state <!-- style-lint: allow --> "No element is currently selected. Please select an element to view its properties and make edits."
...
2.  **Additional Entry Data**: any extra entries you pinned to the composition from the Page Data view (see below). Tooltip: "These entries are directly pinned to this section. Their data is always rendered regardless of which template uses the section."
...
2.  Click the **Pinned Entries** card (or, if entries are already pinned, the **Link Entry** button in the Additional Entry Data accordion, test ID link-entry-button).
...
6.  Choose the **Additional Entry Data** source. Expand the entry you pinned and pick a field. Studio writes the binding and the canvas re-renders with the field's value.
...
Some properties carry a small breakpoint tag next to their label. These are **design props**: visual settings (padding, gap, corner rounding, and so on) that can hold a **different value per breakpoint**. The control always edits the breakpoint the canvas is currently showing:
...
Each breakpoint's value can also be bound to data via the database icon, exactly like other properties: one source (manual value or binding) per breakpoint. Whether a property is a design prop is decided by the developer who registered the component ([developer guide](/docs/studio/design-props)).
...
-   [Composition overview](/docs/studio/what-is-a-composition): where the Data tab sits in the larger authoring flow.
