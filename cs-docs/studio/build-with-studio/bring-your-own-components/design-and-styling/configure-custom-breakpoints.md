...
-   **Unique id and displayName** across all entries.
...
1.  Resizes the canvas iframe to previewSize.width by previewSize.height.
2.  Activates that breakpoint's slot in the Design panel: design changes the author makes are scoped to that breakpoint (or cascade down from the default if the property has no per-breakpoint override).
3.  Updates the rendered composition so authors see exactly what visitors see at that viewport.

This is the core of Studio's responsive workflow: per-breakpoint padding overrides, font-size scaling, hidden-on-mobile flags, all single clicks in the right panel, no media-query CSS authored manually.
