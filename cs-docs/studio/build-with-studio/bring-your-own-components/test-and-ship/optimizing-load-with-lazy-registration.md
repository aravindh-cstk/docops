...
| One component adds a heavy library (charts, rich-text editor, code editor) | Yes |
...
| You have 10 to 20 simple components | Eager is fine |
...
Functionally identical to registerLazyComponent. Same lazy chunk, same bundle behaviour, same Studio integration, but with your own Suspense wrapper.
...
When you're rendering with server-side rendering (SSR) / RSC / static site generation (SSG) (see [CSR vs SSR](https://studio-documentation.contentstackapps.com/prompts/configure-csr-vs-ssr.html)):
...
Internally Studio tags lazy registrations with a \_lazy: true flag on the registry entry. This isn't something you set or inspect. It's there so Studio's tooling can check whether the component is loaded yet and fire the lazy load when needed.
...
-   [CSR vs SSR](https://studio-documentation.contentstackapps.com/prompts/configure-csr-vs-ssr.html)
