...
description: "Reference for every prop type available in Contentstack Studio component schemas, including type-specific options, validation, CSLP tags and the $ props that make bound values editable, and wrap vs studioAttributes patterns."
...
**Slot props are the most important prop type.** Every other type is a value (string, number, image, and so on). A slot is a **drop-zone** where authors place other components in Studio's canvas. If you've read one thing on this page, read this section.
...
-   **Component Slot first, escalate to Section Slot only when Templates need to make the call.** The default rule: [the-studio-page.md, Step 3](/docs/studio/standard-studio-page-anatomy#step-3-meet-slots-the-two-extension-points).

Newcomer walkthrough: [Quickstart 2: Register a component with a Slot](/docs/studio/quickstart-register-a-component-with-a-slot). Full slot type reference (dynamic slot count, children shorthand, edge cases): [section slot below](#component-schema-prop-types-10).
...
| design | boolean | Marks a choice/string prop as a **design prop**: the key must be a camelCase CSS property, values are CSS values editable per breakpoint, and the component receives a generated class instead of the value. Ignored (with a warning) on other types, nested props, and reserved/non-CSS keys. Full guide: [Design props](/docs/studio/design-props). |
...
| "slot" | Drop target on the canvas | Child components, produces a Section-Slot-like placeholder |
...
> **Component Slot vs Section Slot.** A slot prop declared here opens a drop target at **section authoring** time: the section author fills it, and whatever's inside is locked into the section on save. To let the template author fill a region per instance, the section's design must place a [**Section Slot**](/docs/studio/section-slots) inside this component slot. Two authoring layers, two primitives: see [Section Slots, Slot vs Section Slot](/docs/studio/section-slots#slot-vs-section-slot-two-authoring-layers).
...
-   A **string** appears as an error in the right panel, and the value is rejected.
-   false rejects the value without an error message (use a string instead for UX).
-   true or undefined accepts the value.
