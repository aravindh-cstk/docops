...
"Component Default Data" in Studio is two related things that share a name. Both feed values into a component without going through a Contentstack entry, and the one that does more is also the easiest one to miss.
...
This page covers both: schema defaults first (the simpler case), then runtime Component Default Data (the one that resolves values at runtime).
...
The data prop is a React prop on <StudioComponent />: it works wherever React renders the component. The fetch path for specOptions differs by framework (hook in CSR, server function in SSR), but the data prop attaches the same way in both.
...
// The three ids that mean "no page at this URL". Module scope: built once.
...
export default async function ProductPage({ params, searchParams }) {
  ...
  try {
    ...
    // Only a genuine miss is a 404; a pricing outage or an expired token must
    // not be served as one.
    if (NOT_FOUND_IDS.has(error?.id)) return notFound();
    throw error;
  }
...
| Next.js Pages Router | getServerSideProps calls csStudio.fetchCompositionData | Same getServerSideProps (server-side) | Page component receives both as props |
...
They pick livePricing.unit\_price. The composition renders with whatever your pricing API returned. That number is never stored in Contentstack, but the page receives it.
...
-   **Feature flags**: data={{ flags: { newCheckoutEnabled: true } }}, then bind a Condition Block to it
...
Fetching directly inside a registered component works, but it locks the data dependency into the component code: every component requiring live pricing has to import the pricing client and replicate auth, error handling, caching, and SSR concerns.
...
-   [Bring your own data](/docs/studio/bring-your-own-data): rendering a composition against data you hold (<StudioComposition />) and per-slot data (<Slot>)
