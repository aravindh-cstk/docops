...
-   One-off marketing pages where the design lives in Figma
-   Prototyping a layout that matches a design before deciding which components it'll use
-   Quickly turning a design system frame into a Studio composition
...
-   **Keep frames at reasonable sizes.** 1440 by N for desktop, 375 by N for mobile. Studio uses the frame's width as the artboard size, so a frame wider than typical screen widths produces an oversized canvas.
