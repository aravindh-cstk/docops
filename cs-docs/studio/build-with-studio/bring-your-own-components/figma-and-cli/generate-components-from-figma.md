...
| --- | --- |
| You have a Figma design system / component library | Yes |
| You want first-class React components in your repo (committed, version-controlled) | Yes |
| The same Figma component should ship as one Studio component used many times | Yes |
| You're building out an enterprise component library from scratch | Yes |
...
csdx studio:component:register --component-path=<path>

or

csdx studio:component:register --component-dir=<dir>

 | Adds a **local or custom component** to Studio so authors can drop it on the canvas. Use this for components you wrote manually, modified after generation, or need registered in Studio without running the plugin. | Stage 3, after Add (or when you hand-author a component) |
| (auxiliary) **Design token: add** | csdx studio:design-token:add --design-token-id=<id> | Imports design tokens (colours, typography, spacing, effects) from the Figma plugin's Design Token export and writes a register-design-tokens.ts file that you then import from your app entry point. | Stage 3, once per Design Token ID the plugin produced |
...
1.  In Figma, open **Resources**, select **Plugins**, and select **Browse plugins in Community**
2.  Search for **Contentstack Studio** (plugin id 1541766192464484605)
3.  Install, then click **Run**
4.  Pick your organisation's region, click **Authorize**. The plugin opens an Open Authorization (OAuth) window, completes the sign-in, and returns to Figma
...
5.  **Click Generate Design.** The plugin produces:

    -   A **Component ID:** feed this to csdx studio:component:add to generate React source
    -   A **Design Token ID** (when tokens were involved): feed this to csdx studio:design-token:add
    -   **Composition JSON:** pastable directly onto a Studio canvas via Copy to Studio (the [copy/paste](/docs/studio/copy-and-paste-from-figma-to-studio) flow)
    -   **CSS variables:** usable in external stylesheets via Copy CSS Variables
...
| **Copy CLI Command** | Puts a ready-to-run csdx studio:component:add --component-id=<id> on the clipboard | When the Figma design includes a component that **doesn't have generated code yet, or needs refinement** (colour, spacing, behavior tweaks) before adding to Studio. Useful for advanced JavaScript components when you want committable React source. |
...
1.  **Include Studio's default design tokens?** (Y/N)
    ...
    -   **Y:** include Studio's default tokens alongside the tokens in your project files
    -   **N:** import only the tokens defined in your project files
2.  **Pick an access level** for what content editors can use on the canvas:

    -   **dynamic:** editors can only use design options coming from the entries linked to the Studio project. They cannot access or apply custom colours or other styling available in the Studio editing canvas.
    -   **tokens:** includes access to the dynamic components AND the custom design tokens added through the CLI
    -   **arbitrary:** extends access beyond dynamic and CLI-added tokens, allowing editors to use any design properties available in the Studio canvas (custom colours, styles, visual adjustments)
3.  **Output path:** defaults to ./src/config/register-design-tokens.ts inside your project.
...
![Terminal showing csdx studio:design-token:add generating the register-design-tokens.ts file, interactive prompts ask whether to include Studio](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/amc8c90159069e3dfd/226a7a0f6d2bb4ea1a746aef/figma-cli-design-token-generated.png)
...
Register adds a **local or custom component** to Studio for reuse. The component can be a local component or a custom component:
...
-   A component you **hand-wrote** (not from Figma): Register is the path to expose any local React component to Studio, whether the plugin was involved or not
...
![Figma](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/am5eb2f914bc27e98d/d8527ab583091560fdbb80e6/figma-best-image-export.png)
...
| Verify the Figma file is **editable** before generating | Read-only files block Auto Layout updates, token adjustments, and component mapping |
...
4.  **Tokens:** if csdx studio:design-token:add generated new tokens, review whether to keep them or rename them to fit existing names in your system.
