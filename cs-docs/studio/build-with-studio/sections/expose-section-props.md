...
> **Compound-component framing.** A section is a compound component you built in Studio. Exposed Props + [Section Slots](/docs/studio/section-slots) are its two surfaces to the template author: props control which **values** they can change, Section Slots control which **regions** they can drop into. See [Section Slots: Sections are compound components](/docs/studio/section-slots#sections-are-compound-components-section-slots-exposed-props-give-them-the-shape) for the full framing.

Sometimes you don't want to expose a whole region (that's what [Section Slots](/docs/studio/section-slots) are for). You want to let the template author tweak **a single value** inside the section per instance. The headline text. A CTA label. An image source.
...
In the Card Grid section's Save flow, only the Header's Text prop is toggled exposed (renamed to **"Card Title"**). The Heading Level, the Repeater's Items, and the Card's Title are left internal. Templates that drop Card Grid see Card Title in their right panel. Nothing else from the section is overridable.
...
-   [Spec shape: Exposed section prop](/docs/studio/section-compositions-api#case-14-exposed-section-prop): the stored sectionExposedProps schema
