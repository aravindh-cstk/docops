...
2.  **Per-query option**: csStudio.fetchCompositionData({ url }, { locale: "fr-fr" }) overrides the default (locale lives in the second-arg CompositionQueryOptions, not in the query itself). This is the recommended way to carry locale into Studio fetches on visitor routes.
3.  **Project default Language**: what you set in [configure-studio](https://studio-documentation.contentstackapps.com/prompts/configure-studio.html).
...
### Resolve locale from your routing layer. Pass it to the SDK
...
-   **Per-locale caching.** Different routing-layer URLs (path / subdomain / cookie + cache key) already produce separate content delivery network (CDN) cache entries. Locale-aware caching works without involving Studio.
-   **Pattern stability.** Your Studio URL pattern stays focused on the content shape (e.g. /blog/{{entry.slug}}). The locale dimension is orthogonal and lives in routing.
-   **Avoids the editor and visitor split.** Studio's editor preview already picks locale via its navbar (Layer 1 above). Having a separate visitor mechanism that doesn't encode locale into the Studio pattern keeps preview and visitor render aligned.

### Fallback locale chains

Contentstack supports per-locale fallback\_locale. Studio honors the chain. To configure it, open Stack, then Settings, then Languages. Each locale has a fallback (e.g. fr-ca falls back to fr-fr falls back to en-us).

What this returns: if an entry isn't translated to fr-ca yet, Studio resolves bindings against the fr-fr version automatically. Authors don't see broken pages. They see French content until Canadian-French is ready.

**Pitfall:** confirm the chain ends at a locale that's always translated (your default). An unterminated chain renders empty fields.
...
| Hard-coding the locale in the canvas-app's stack init | Studio overrides at iframe-load time, but csStudio.fetchCompositionData calls don't: bindings resolve against the wrong locale |
...
-   **Right-to-left rendering** isn't a Studio concern. Your components are responsible for RTL via your design system. Verify registered components honour dir="rtl" for Arabic/Hebrew locales.
...
-   [Configure Studio](https://studio-documentation.contentstackapps.com/prompts/configure-studio.html): set the project default Language
