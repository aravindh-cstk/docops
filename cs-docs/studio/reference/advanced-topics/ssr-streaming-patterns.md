...
Modern SSR frameworks (Next App Router, Remix with defer, Astro islands) stream the response: the shell goes out first, slower content streams in chunks. Studio composes pages from many bindings, each of which may resolve to a Content Delivery API (CDA) call. Naive integration blocks the entire response on the slowest call. Done right, the shell + above-the-fold renders fast and the below-the-fold sections stream in.
...
// fetchCompositionData REJECTS for a not-found URL — it does NOT resolve with
// { hasSpec: false }. These three ids mean "no page here"; anything else is a
...
-   [Configure CSR vs SSR](https://studio-documentation.contentstackapps.com/prompts/configure-csr-vs-ssr.html): pick the render path that matches your framework
