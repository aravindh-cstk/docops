...
2.  **Visitor site (you set this).** Your route passes variantAlias explicitly to the SDK as a query option, the **second argument** (CompositionQueryOptions), never a field of the query object and not a studioSdk.init() key:

    -   Server-side rendering (SSR): csStudio.fetchCompositionData({ url }, { variantAlias: "holiday-2026" })
    -   Client-side rendering (CSR): useCompositionData({ url }, { variantAlias: "holiday-2026" })
    -   variantAlias is NOT a studioSdk.init(...) config field. The SDK's UserConfig only accepts stackSdk, contentTypeUid, locale, cslp, openInStudioButton, fetchComposition, fetchTemplateEntry. Pass variantAlias per fetch, not on init.

If no variant is specified at either path, the base entry renders.

Variants are different from locales. Locale = "this content in another language." Variant = "this content for a different audience/moment." Use both together when needed (fr-fr combined with holiday-2026 = the French holiday variant).

## Recommended patterns

### A/B testing through variants

Two entry variants, same template, server-side variant selection. The variant flows in via the **variantAlias option** (the second argument to csStudio.fetchCompositionData) and the resolved specOptions is the object you pass straight to <StudioComponent />:
...
export default async function ProductPage({ params }) {
  ...
  const specOptions = await csStudio.fetchCompositionData(
    // 1st arg — the query. searchQuery forwards the request query string
    // (Studio uses it for iframe overrides); "" when there's none to forward.
    { templateContentTypeUid: "product", searchQuery: "" },
    // 2nd arg — options
    {
...
Authors edit variant B as another entry view. No code change to "ship variant B": that's pickExperimentVariant returning "B" for more traffic.
...
-   **Editor preview of "what visitors see" for a specific persona.** The navbar variant picker tests one variant at a time. There's no single view of "preview as Persona X." Workaround: switch variant in the navbar and use the entry picker to select the variant-targeted entry.
...
-   [Configure CSR vs SSR](https://studio-documentation.contentstackapps.com/prompts/configure-csr-vs-ssr.html): variant resolution differs between paths
