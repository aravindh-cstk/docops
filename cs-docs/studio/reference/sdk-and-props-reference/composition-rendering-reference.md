...
## StudioComponent reference: everything you can pass
...
type StudioComponentSpecOptions = {
  spec: StudioSpec | null;          // the resolved composition; a miss rejects, so this is set
  fetchOptions: StudioComponentFetchOptions;  // what was used to fetch
  hasSpec: boolean;                 // deprecated; always true on a resolved call
  hasTemplate: boolean;             // deprecated; false for every freeform composition
...
-   **Server-side rendering (SSR)**: await csStudio.fetchCompositionData(query, options?)
-   **Client-side rendering (CSR)**: useCompositionData(query, options?) returns { specOptions, isLoading, error, refetchSpec, refetchData }
...
Use this for **anything not in Contentstack**: pricing APIs, A/B variant flags, geolocation, weather, user profile, URL search params. See [Component Default Data](/docs/studio/set-component-default-data) and the [wire-external-data](https://studio-documentation.contentstackapps.com/prompts/wire-external-data.html) skill.
...
-   **404 handling lives on the thrown error, not on specOptions.** fetchCompositionData rejects when nothing matches, so a resolved call always means render. Branch on the ComposableStudioError's id: COMPOSITION\_NOT\_FOUND, COMPOSITION\_NOT\_FOUND\_BY\_URL and PREVIEW\_ENTRY\_NOT\_FOUND are your 404. Every other id is a real failure and belongs in the 5xx family. hasSpec and hasTemplate are **deprecated** and are not not-found signals: hasTemplate is false for every **freeform** composition, so gating a 404 on it 404s every freeform page you publish.
...
-   [<StudioComposition /> reference](/docs/studio/studio-composition-props-reference): render a composition against data you hold (bring-your-own-data)
-   [Component Default Data](/docs/studio/set-component-default-data): the canonical data prop reference
-   [Wire external data](https://studio-documentation.contentstackapps.com/prompts/wire-external-data.html): skill that wires runtime data through the data prop
-   [Configure CSR vs SSR](https://studio-documentation.contentstackapps.com/prompts/configure-csr-vs-ssr.html): pick fetchCompositionData (SSR) vs useCompositionData (CSR)
