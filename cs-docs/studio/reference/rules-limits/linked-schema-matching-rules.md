...
The child names can differ. Studio builds a positional remap (e.g. section's heading maps to page's title) when names don't match.

![Four matching examples for group / global_field fields, identical multisets match (order-invariant). Different child counts or types do not](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/am00719dc6d5c2c829/c45eaafb8bbb9e9f89f2aa1b/reference-group-matching-examples.png)
...
![Three rows showing the modular blocks intersection rule, any shared block UID declares a match. Disjoint unions do not](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/amead03429c141fe3b/61714ba89d03b76e32543cdd/reference-blocks-matching-examples.png)

Why looser: sections typically wrap their Repeater in a ConditionBlock that narrows to one specific block UID, so they care that the targeted block exists on both sides, not whether the unions are equal. Strict set equality rejects the common recursive-shape pattern where the template field's nested modular\_blocks allows only the inner block type while the section's modular\_blocks allows several at root.
...
![Four rows showing strict sorted-set equality for reference fields, order-invariant equality matches. Subsets, supersets, or disjoint unions do not](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/ama33237bb28e93da7/7102c45ee4621f4f196572df/reference-references-matching-examples.png)
...
![Three rows showing scalar matching: same data_type and same multiple flag matches. Differences in either do not](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/am9eb213ab49551b0a/1193b290db9ab1db68a153ce/reference-scalar-matching-examples.png)
...
| --- | --- |
| **Exactly 1** | Auto-picked. The section binds to that field without user input. The **Data root** control in the right-panel **Settings** shows what it is reading from (the control is the same on every section drop). |
| **More than 1** | Studio binds to one (the first in schema order). The **Data root** picker offers the other matches. The author can switch. Per-instance: dropping the same section twice on the same template lets each instance read from a different field. |
...
![Section selected on a template canvas: right-panel Settings shows the Data root the section is reading from. The same control appears on every section drop. The picker offers more targets when several match.](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/am1dd710418fc692c5/62f6f6d243bcf621f557df4a/section-data-source-dropdown.png)
...
![Section schema heading/body/image remapped by position to Page schema title/desc/hero via three indigo arrows](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/am71d34cdde63c355a/e41db601bf7d90c31cfb053a/reference-positional-remap-example.png)
...
![Three schema cards demonstrating the shallow nested-group remap: Page A matches because the nested group keeps the name](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/am38b018be8413d5f5/b8c965a018d7c1cfeb6e2f3a/reference-nested-group-remap-cases.png)
