...
-   **Bind first, customise later.** Drop a component, bind every prop that's content, and only then style it. Builds in the data dependency from the start.
...
-   **Slug fields drive URL patterns.** Confirm every linked CT has a URL-shaped field (url, slug, etc.) and use it in the template's URL pattern.
...
-   **Use server-side rendering (SSR) or static site generation (SSG) for visitor pages.** Pure client-side rendering (CSR) is fine for the canvas route, weak for SEO on real pages.
