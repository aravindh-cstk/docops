...
**Playground Canvas** is a built-in Studio canvas that lets you author compositions _without setting up a canvas-app in your own codebase_. Studio hosts the iframe; the playground ships with Studio's default components; no <StudioCanvas /> route, no Canvas URL configuration, no SDK install in your app.
...
![Decision — Canvas URL set: Website Canvas (needs the Studio SDK); not set: Playground Canvas (Studio's hosted iframe, no SDK required)](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/am30b90d5db50c5a6c/12785582eec19ccfdb39efde/setup-playground-vs-website-canvas.svg)
...
| Drop your own registered components | ❌ — Studio's playground iframe has no access to your component registry | ✅ — Studio loads your SDK + components from your app |
...
-   **No external [Component Default Data](/docs/studio/set-component-default-data).** The data prop on <StudioComponent /> is set by your app — without an app, there's no place to pass external pricing / variants / feature flags from. You can still bind to CMS entries via the Data Picker; just no external-data hook yet.
...
The transition is one-way: once the Canvas URL is set, Studio uses it for all canvas previews on this project. You can clear the Canvas URL to drop back into Playground mode, but typical workflow is "Playground → Website Canvas, then stay there."
...
-   Bring your own components — once on Website Canvas, your components show up here
