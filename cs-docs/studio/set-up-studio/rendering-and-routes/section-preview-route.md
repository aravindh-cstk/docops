...
4.  Set the same path as the **Canvas URL** in Studio project Settings, then Configuration
...
Now point Studio at the path you picked. Open your project, go to Settings, then Configuration, and set **Canvas URL** to your path (e.g. /canvas).

Studio combines the **Base URL (origin) + Canvas URL (path)** to get the full iframe URL. The Base URL is **not** the Canvas URL field. It's the per-locale URL on the environment the project targets. To find that value, open your stack, go to Settings, then Environments, then <env>, and read the URL for <locale>. For local dev that must be your dev origin:
