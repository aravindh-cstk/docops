...
Studio doesn't force a render model on you. The SDK returns two ways to fetch a composition and two ways to render it. You pick the combination that fits your framework.
...
Marked "use client": works inside an SSR or RSC tree because React pre-renders client components into HTML during SSR. The directive means it hydrates on the client and has access to browser APIs after that.
...
When you call useCompositionData on the client, the SDK reads window.location.search itself. There's no parameter to pass. It reads the query string automatically.
...
The SDK reads searchQuery to apply Studio's iframe overrides (preview entry, locale, variant, draft content) and Live Preview's draft hash. The specific param names are an internal contract between Studio and the SDK: they may change between SDK versions and your code must not parse or rely on them.
...
| Framework | How |
| --- | --- |
| Next.js App Router (Server Component) | searchParams prop on the page, serialised back to a string with new URLSearchParams(searchParams).toString() |
...
export default function BlogPostPage({ slug }) {
  ...
  // Narrowing only: after isLoading and error are handled, specOptions is set.
  // This is not the 404 decision — with the hook that lives in `error` above.
...
-   No SEO without extra work (search engines won't see the rendered content)
-   First paint shows the loading state, measure if that's acceptable for your UX
...
// The only three error ids that mean "no page at this URL". Anything else is a
// real failure and must stay a 5xx.
...
export async function getServerSideProps(context) {
  ...
  // fetchCompositionData REJECTS for not-found URLs — catch and classify the
  // error id to return a 404 cleanly. There is no empty-but-resolved state to
  // guard against: a resolved call always carries a spec.
  try {
    ...
    if (NOT_FOUND_IDS.has(error?.id)) return { notFound: true };
    throw error; // a real failure must stay a 5xx
  }
}
...
-   searchQuery is required: pass it from context.req.url
-   specOptions is a JSON-serialisable object: Next.js will serialise/deserialise it through getServerSideProps cleanly
-   On the client, **wire Live Preview's onEntryChange** to refetch (see below), otherwise the page stays stale when authors edit content
...
-   Don't try to render <StudioComponent /> in the Server Component: Next.js will complain
-   searchParams in App Router pages is an object. Serialise it back to a query string with URLSearchParams
-   Hydration mismatches won't happen if your server-fetched specOptions is identical to what the client would fetch. Both go through the same SDK path, so they are, as long as searchQuery is forwarded correctly.
...
export async function getStaticProps({ params }) {
  ...
  try {
    ...
    // fetchCompositionData rejects if the URL doesn't resolve — but only these
    // ids mean "no page here"; anything else is a real failure.
    if (NOT_FOUND_IDS.has(error?.id)) return { notFound: true };
    throw error;
  }
}
...
-   searchQuery: "" is safe at build time, no Studio iframe is involved
-   ISR revalidation handles publish updates. For instant updates, wire a Contentstack publish webhook to res.revalidate() on a custom API route.
-   Live Preview won't update an SSG page until revalidation triggers: the client-side onEntryChange only works after hydration
...
If you forget dynamic({ ssr: false }) in App Router, you'll see a React hydration error: <Suspense> (server) vs <div> (client). The bare "use client" doesn't fix this by itself.
...
| Use <StudioCanvas /> on a template route | Studio routes every URL to a section, so nothing renders | Use <StudioComponent /> on template routes. <StudioCanvas /> only on the canvas route |
