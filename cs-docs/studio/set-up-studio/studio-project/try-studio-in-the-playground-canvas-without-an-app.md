---
title: "Playground Canvas: try Studio without setting up an app"
...
![Decision. Canvas URL set: Website Canvas (needs the Studio SDK). Not set: Playground Canvas (Studio](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/am246dbb8482db88b1/620bde17c3981e59f222700f/setup-playground-vs-website-canvas.png)
...
| --- | --- | --- |
| Drop Studio's built-in components onto the canvas | Yes | Yes |
| Drop your own [registered components](/docs/studio/bring-your-own-components-guide) | No: Studio's playground iframe has no access to your component registry | Yes: Studio loads your SDK + components from your app |
| Build sections and templates | Yes | Yes |
| Bind components to real CMS data (entries, references) | Yes | Yes |
| Save compositions | Yes | Yes |
| **Deploy** | No: disabled in playground | Yes |
| Preview the composition against a real visitor URL | No: the playground is the only preview | Yes, via your template preview routes |
...
-   **No external [Component Default Data](/docs/studio/set-component-default-data).** The data prop on <StudioComponent /> is set by your app. Without an app, there's no place to pass external pricing / variants / feature flags from. You can still bind to CMS entries via the Data Picker, but there's no external-data hook yet.
...
4.  [Register your components](/docs/studio/bring-your-own-components-guide) in your canvas-app so they appear in Studio's palette
...
| Confusion between Playground (Studio-hosted) and your canvas-app (<StudioCanvas /> route) | Authors don't know which iframe they're looking at | Studio's loading message reads "Using playground environment" when on Playground. The canvas top bar surfaces a "Configuration" gear that lets you check + set the Canvas URL |
...
-   [Bring your own components](/docs/studio/bring-your-own-components-guide): once on Website Canvas, your components show up here
