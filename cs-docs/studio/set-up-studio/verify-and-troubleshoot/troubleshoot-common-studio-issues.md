...
3.  **In DevTools on the Studio tab, open Network, then reload the composition. What's the status code of the request to your Canvas URL?**

    -   200 and content returned: a Studio-side rendering issue. See [Runtime render](#runtime-render) or [Composition load](#composition-load).
    -   net::ERR\_BLOCKED\_BY\_LOCAL\_NETWORK\_ACCESS\_CHECKS / blocked:mixed-content / blocked:private-network-access: browser blocking. See [Studio project + canvas](#studio-project-canvas).
    -   401 / 403: auth. See [SDK initialization](#sdk-initialization).
...
**Cause:** Studio SDK requires Node 18 or later.
...
**Cause:** older SDK version. The /rsc subpath entry ships from @contentstack/studio-react 1.4 or later.
...
Handled in [jsdom + JSON-RTE in Node](/docs/studio/troubleshoot-common-studio-issues#node-ssr-from). Short answer: htmlToJson needs a real DOM HTMLElement, not a string. Wrap it with jsdom in Node contexts.
...
studioSdk.init was never called, or it ran after useCompositionData first rendered.

**Fix:** Init must complete before any hook call reaches the SDK. The safest place is a shared module imported at app boot (usually src/lib/contentstack.ts) so React mounts no hook call before init finishes.
...
variantAlias is not a valid studioSdk.init() config field. The SDK's Zod schema silently strips unknown keys, so the call doesn't throw. The variant never applies. **Runtime-verified.** UserConfig accepts only: stackSdk, contentTypeUid, locale, cslp, openInStudioButton, fetchComposition, fetchTemplateEntry.
...
1.  In your stack, open Settings, then Tokens, then the Delivery Token edit screen.
2.  Confirm your .env.local has the right values.
3.  Restart your dev server: env vars are read at boot.
...
1.  Create the Studio project in app.contentstack.com (in Studio, open New Project, then link your stack). Provisioning creates the compositions content type.
2.  Note the UID. In Studio, open your project, go to Settings, then Configuration, and read "Composition Content Type". If it isn't compositions, set the corresponding env var in .env.local.
3.  Until a composition is published on a route you're hitting, wrap the fetch in try/catch and render a "no composition yet" placeholder. The 422 goes away once any composition is published.
...
**Fix:** Confirm studioSdk.init({ contentTypeUid: "..." }) matches the **Compositions** CT in your stack. In Studio, open your project, go to Settings, then Configuration, and read "Composition Content Type".
...
1.  Open your project, go to Settings, then Configuration, and set **Canvas URL** to your route path (e.g. /canvas).
2.  Confirm that route exists in your app and mounts <StudioCanvas />.
3.  See [Section preview route](/docs/studio/section-preview-route).

### "Environment Not Configured"

Studio can't resolve the Base URL because no Environment + Language is picked.

**Fix:** Open your project, go to Settings, then Configuration, and pick both.
...
![Studio canvas showing the SDK Not Initialized dialog with three potential-issue bullets and an SDK Initialization Code snippet. The iframe body behind the dialog is empty because the browser blocked the http://localhost load](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/amdb21625379cc1821/1f9c1ea7915315af2083853d/troubleshoot-sdk-not-initialized-pna.png)
...
**Note:** related to the "studioSdk is not initialised" entry in section SDK initialization. The Studio-side symptom is identical, but the root cause here is a browser-level network block, not a missing init() call.

**Underlying cause.** Studio is served from https://app.contentstack.com (public HTTPS). Your dev server is http://localhost (private, plain HTTP). Every modern browser rejects the private HTTP iframe inside a public HTTPS page. Two overlapping mechanisms:

1.  **Mixed content**: browsers reject http:// subresources from an https:// page. Blocks Safari, Firefox, and any browser without PNA.
2.  **Private Network Access (PNA)**, Chromium-only extra check: even HTTPS subresources going from a public page to a private address need opt-in. Brave enforces by default, Chrome / Edge behind a flag.
...
| --- | --- |
| **Any browser (Studio-side)** | "SDK Not Initialized" overlay dialog |
| **Brave** | Inside iframe: "The connection is blocked because it was initiated by a public page to connect to devices or servers on your local network." Network: net::ERR\_BLOCKED\_BY\_LOCAL\_NETWORK\_ACCESS\_CHECKS |
| **Chrome / Edge** | "Mixed Content: The page at 'https://app.contentstack.com/…' was loaded over HTTPS, but requested an insecure resource 'http://localhost:...'. This request has been blocked." |
| **Safari** | Empty iframe. Console: "\[Warning\] Blocked mixed active content 'http://localhost:...'" |
| **Firefox** | Empty iframe with shield icon. Console: "Blocked loading mixed active content 'http://localhost:...'" |

**The one fix that works in every browser.** Serve your dev server over HTTPS with a locally-trusted certificate. See [**Serve your canvas-app over HTTPS locally**](/docs/studio/serve-your-canvas-app-over-https-locally), a full walkthrough with per-framework configs for Vite, Next.js, Remix, Astro, Nuxt, Angular, Webpack, CRA, custom Node/Express, and Docker Compose (~5 min, one-time setup per machine).

**Per-browser dev-only escape hatches**: re-enable defaults when you're done:

| Browser | Escape hatch |
| --- | --- |
| **Brave** | In brave://flags, set "Block insecure private network requests" to **Disabled**, then relaunch |
| **Chrome / Edge** | Set chrome://flags/#block-insecure-private-network-requests to **Disabled**, then relaunch. Also click the shield icon, open **Site settings**, then **Insecure content**, and select **Allow**. |
| **Safari** | No per-flag toggle. Use mkcert HTTPS above, or switch browsers for Studio |
| **Firefox** | In about:config, set security.mixed\_content.block\_active\_content to false (global, flip back after) |
...
**Note:** related to the "Use Studio Canvas Component" overlay entry in section Studio project + canvas (above). Same symptom described from the renderer-mix-up angle. Both entries retained.
...
**Runtime-verified.** Navigating a Vite React-Router SPA to an unregistered URL falls back to the SPA shell HTML but React Router matches no route, which leaves an empty body, so visitors see nothing. Studio itself surfaces this as a "Template Did Not Load" banner with the composition UID and failed URL pattern.
...
**Runtime-verified.** useCompositionData({ compositionUid: "blt2ec5a39fff004468" }) (entry UID) returns "Composition not found. The requested composition does not exist or has been deleted." The same call with the slug, useCompositionData({ compositionUid: "hero\_strip" }), returns hasSpec: true.

**Fix:** compositionUid accepts the composable\_uid **slug**, the value in the entry's composable\_uid field (e.g. "hero\_strip", "pdp\_marketing\_band", "blog\_post\_template"), NOT the entry UID (blt…). In Studio, open your composition and go to the right-panel Settings, where "Composable UID" is shown. You can also read it via curl against the compositions CT.
...
**Runtime-verified.** csStudio.fetchCompositionData({ url: "/blog/does-not-exist" }) **rejects the promise** with "Entry not found for content type: blog\_post". It does NOT resolve with { hasSpec: false }. If you don't handle it, your framework treats it as a server error and renders a 500.

**Fix:** Wrap in try/catch, classify the error id, and call the framework's not-found API. Do not add a hasSpec check as a "backstop": there is no empty-but-resolved state for it to catch. Same rule, stated once for all frameworks: [Rule 5: wrap the fetch](/docs/studio/framework-recipes#rule-5-wrap-the-fetch-or-every-unmatched-url-is-a-500).
...
try {
  ...
  // Next App Router; or return { notFound: true } for getServerSideProps
  if (NOT_FOUND_IDS.has(error?.id)) notFound();
  throw error;  // a real failure must stay a 5xx, not become a 404
}
return <StudioComponent specOptions={specOptions} />;
```
...
**Fix paths (in order of likelihood):**

1.  **Add a catch-all route** mounting <StudioComponent />, recommended default. See [Composition load: Templates 404](#templates-404-in-production-or-template-did-not-load-in-studio-even-though-the-c).
2.  Edit the template's URL pattern in Studio: click the pencil icon in the canvas toolbar to open the Edit URL modal (see [Connected content type: Editing the URL](/docs/studio/connected-content-type#editing-the-url)).
3.  Confirm the route mounts <StudioComponent /> and not <StudioCanvas />. They're not interchangeable.
...
The composition spec references a component type string that doesn't match anything registered. **Runtime-verified.** Walking a resolved specOptions.spec.ui tree shows every type the composition requires. If a type isn't registered, the SDK renders a fallback.

**Fix:**

1.  In DevTools console, dump the type names in the spec:

    ```
    const { specOptions } = useCompositionData({ url });
    const types = new Set();
    const walk = n => { n?.type && types.add(n.type); (n?.children ?? []).forEach(walk); Object.values(n?.slots ?? {}).forEach(s => (s ?? []).forEach(walk)); };
    walk(specOptions.spec.ui);
    console.log([...types]);
    ```

2.  Compare with your registerComponent calls. Add a registration for any missing type.
3.  If the case is off (Hero vs hero), the type strings must match exactly.
...
**Runtime-verified.** With ContentstackLivePreview.onEntryChange(refetchSpec) wired to useCompositionData's refetchSpec, firing the callback triggers a fresh Content Delivery API (CDA) fetch of the composition + template + entry, confirmed via Network tab (three new requests appear immediately after each callback fire).

**Fix:**

1.  Confirm ContentstackLivePreview.init({ … }) runs at app boot.
2.  Wire onEntryChange to your render or refetch:

    ```
    const { refetchSpec } = useCompositionData({ url });
    useEffect(() => {
      ContentstackLivePreview.onEntryChange(refetchSpec);
    }, [refetchSpec]);
    ```

3.  Confirm your stack has Live Preview enabled stack-wide (in your stack, open Settings, then Visual Experience, then General).
4.  Check the Preview Token in your .env.local matches the one on Contentstack.
5.  See [Install Live Preview](/docs/studio/install-live-preview).
...
Then describe what you're seeing, e.g. "my canvas iframe is blank", "edits don't reflect in real time", "401 on every fetch", "'Either compositionUid, url, or templateContentTypeUid is required' error". The LLM walks the same diagnostic tree and offers to fix what it finds.
