...
-   [Partial Prerendering (PPR)](https://nextjs.org/docs/15/app/getting-started/partial-prerendering)
...
Launch supports Next.js Partial Prerendering, available for Next.js 16 and above via the [Cache Components model](https://nextjs.org/docs/app/api-reference/config/next-config-js/cacheComponents).

PPR depends on streaming responses to serve the static shell immediately while dynamic content loads separately.

**Note:** On Launch, you must enable [Streaming response](/docs/launch/streaming-in-launch) mode in the project's Build Settings. The default Buffered mode still deploys a PPR-enabled app without errors, but it holds the entire response until every section, including streamed ones, is ready, which defeats the fast, static-shell benefit that PPR provides.

The same Data Cache limitation applies here. See [Limitations](#limitations) below.
