...
When a Launch server application communicates with external services, its outbound IP address changes dynamically. External services may fail to recognize or authorize requests from variable IP addresses. Static IPs fix this by giving your app a single, permanent address for all its outgoing requests.
...
Launch gives you two ways to manage how your application reaches your backend. Static IPs are the simpler option, whereas Private Network Deployments are the dedicated solution. Here is how they compare.
