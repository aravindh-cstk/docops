...
| res.status(code) | A function to set the status code sent with the response where the code must be a valid [HTTP status code](https://en.wikipedia.org/wiki/List_of_HTTP_status_codes).  
Returns res for chaining. | Response |
...
The Launch Cloud Functions runtime environment leverages [**Node.js**](/docs/launch/supported-nodejs-versions) to power its execution environment.
...
Launch Cloud Functions support the x86\_64 instruction set.
...
![Launch_CloudFunctions_ServerLogs.png](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/bltd0c4e8b609430130/665d9b30dbfae98a3b0f8e1f/Launch_CloudFunctions_ServerLogs.png)
...
If a project with server-side functionality is detected, Launch Cloud Functions will be automatically disabled during deployment. This will be reflected in the [**Deployment Logs**](/docs/launch/deployments#deployment-logs) with the following message:
...
-   If the Cloud Functions have dependency packages, follow the steps below:
    -   Ensure that a package.json file is present at the root directory.
