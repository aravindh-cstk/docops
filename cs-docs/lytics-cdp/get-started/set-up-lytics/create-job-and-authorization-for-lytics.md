...
This guide provides step-by-step instructions for creating a new 'Import Entries' Job along with the authorization for it. Channel tool [**authorizations**](/docs/lytics/keys-authorizations) refer to the permissions granted to various integrations that allow Lytics platform to access and use data from third-party channels.
...
Detailed [job status](/docs/lytics/data-sources#job-status) are provided to understand better what is happening in the background during a job's lifecycle. These states will vary by job but include:
