...
Server-side integrations allow Lytics and the third-party service to communicate directly, usually in the form of a job. These jobs are available to set up and execute in the Lytics platform under [Data > Jobs](https://app.lytics.com/data/jobs). There are two main types of Server-side integrations:
...
File imports and exports can be decrypted and encrypted, respectively using [Pretty Good Privacy](https://en.wikipedia.org/wiki/Pretty_Good_Privacy) (PGP). To create or ingest PGP encrypted files, use an authorization that includes PGP keys.
...
Use a PGP Public Key authorization to encrypt file exports. You will need to [generate an armored PGP key pair](https://www.gnupg.org/gph/en/manual/c14.html) if you haven't already.
...
User profile fields can be dynamically inserted into an alert using templating of supported fields including the alert title, alert subtitle, and alert body fields. Lytics uses [Golang's template package](https://golang.org/pkg/text/template/) to provide this functionality. The data available to you will vary based on the data available on each individual profile.
