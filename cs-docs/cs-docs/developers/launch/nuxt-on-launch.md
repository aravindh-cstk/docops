...
Check out the [Caching Guide for Contentstack Launch](./caching-guide-for-contentstack-launch.md) for more information.
...
**Additional Resource:** Refer to the [Quick Start Guide with Nuxt](./quick-start-nuxt.md) documentation for a step-by-step walkthrough to deploy your Nuxt project on Launch as an SSR-based application.

## Setting up Edge Redirects and Rewrites in Nuxt on Launch

When configuring [Edge Rewrites](./edge-url-rewrites.md) or [Edge Redirects](./edge-url-redirects.md) in Contentstack Launch—either via `launch.json` or using [Edge Functions](./edge-functions.md), it is a prerequisite to use native `<a>` tags instead of `<NuxtLink>` components within your Nuxt application.
...
**Note:** For **error code 500**, please refer to the timed-out errors in the [Server Logs](./cloud-functions.md#server-logs) tab to understand and address the issue.
...
The Launch application runtime environment supports [Node.js version 20](https://nodejs.org/docs/latest-v20.x/api/index.html) to power its execution environment. Learn more about configuring the specific Package Manager [here](./supported-package-managers.md).
...
- **Setting up Cache revalidation with **[**Nuxt internationalization**](https://i18n.nuxtjs.org/docs/getting-started/usage#route-localization)** enabled**
  ...
  In such cases, it is important to [revalidate the cache](./revalidate-cdn-cache.md) for both the localized URL (e.g., `/nl/about-us`) and the base URL (`/about-us`).
