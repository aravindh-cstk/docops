...
url: https://www.contentstack.com/docs/developers/launch/launch-security-features
...
- Apply request validation or routing logic using [Edge Functions](/docs/developers/launch/edge-functions).
- Restrict access to trusted networks using [IP-based access control](/docs/developers/launch/ip-based-access-control-using-edge-functions).
- Restrict access to bot and automated traffic using [AI crawler and bot](/docs/developers/launch/blocking-ai-crawlers) access controls.
- Limit the exposure of backend logic implemented through [Cloud Functions](/docs/developers/launch/cloud-functions).

## IP-Based Access Control

Contentstack Launch supports [IP-based access control](/docs/developers/launch/ip-based-access-control-using-edge-functions) using [Edge Functions](/docs/developers/launch/edge-functions), allowing you to restrict access to applications based on IP address rules.
...
Applications hosted on Launch benefit from **CDN-level bot detection and mitigation**, which helps reduce unwanted automated traffic. Additionally, Launch enables the [blocking or controlling of bot traffic](/docs/developers/launch/blocking-ai-crawlers), including **AI crawlers**, using [Edge Functions](/docs/developers/launch/edge-functions).
...
Contentstack Launch automatically includes [geolocation headers](/docs/developers/launch/geolocation-headers) with incoming requests, providing geographic information such as **country**, **region**, and **city** based on the **visitor’s IP address**.

These headers can be used in [Edge Functions](/docs/developers/launch/edge-functions) or application logic to apply geolocation-based access and routing controls, such as allowing or blocking traffic from specific countries or regions.
...
Contentstack Launch supports [password protection](/docs/developers/launch/password-protection) for environments, allowing you to restrict access to specific deployments using a password prompt.
...
In addition, Launch allows you to apply password protection to specific domains within an environment using [Edge Functions](/docs/developers/launch/edge-functions). This enables more granular access control when an environment is associated with multiple domains.
...
After [deleting a domain](/docs/developers/launch/custom-domain#delete-a-custom-domain) in Launch, **remove all related DNS records** (such as A, CNAME, ALIAS, or TXT records) from your DNS provider to prevent them from inadvertently pointing to your application or exposing unused endpoints.
...
**Additional Resource:** Learn more about [traffic encryption](/docs/developers/launch/traffic-encryption) in Launch.
...
Launch provides secure management of [environment variables](/docs/developers/launch/environment-variables) at the [environment](/docs/developers/launch/environments) level.
...
Use Launch [environment variables](/docs/developers/launch/environment-variables) and follow the listed best practices (store secrets and credentials as environment variables, avoid hardcoding sensitive values, and scope sensitive values only to the environments that require them).
