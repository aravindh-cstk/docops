...
- When you create an **Access Key** pair, you should download and securely store the **Secret Access Key** immediately, as it cannot be retrieved later.**Note**: Each user can have up to two access keys at any time. If you lose your **Secret Access Key**, you must delete the access key and create a new one.
...
- Select the **Bucket** from the top-right drop-down and then click **Choose File(s)** to add new assets.**Note**: You can add up to 10 assets in one go.
