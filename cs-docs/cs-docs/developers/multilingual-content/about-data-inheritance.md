...
Data inheritance is the continuous inheritance from the [Fallback language](/docs/developers/multilingual-content/about-fallback-languages/) until the entry is localized.

Let’s say you wish to create a multilingual site that serves content in several languages: English (United States), French (France), Spanish (Spain), and Japanese (Japan).If you have set English (United States) as the [master language](/docs/developers/multilingual-content/set-the-master-language/), you will need to add French, Spanish, and Japanese manually.

While [adding languages](/docs/developers/multilingual-content/add-a-language) to your [stack](/docs/developers/set-up-stack/about-stack/), you also need to specify the fallback language for each language that you add. Fallback language is the parent from which the given language [inherits data](/docs/developers/multilingual-content/about-data-inheritance/) from. By default, the fallback for each language is the master language. You can change this while adding languages to your stack.

Let’s consider that you have created an [entry](/docs/content-managers/author-content/about-entries) in the master language (i.e., English (United States) in our case). When you start [creating an entry](/docs/content-managers/author-content/create-an-entry/) in any of the added languages (e.g., French - France), it will show data inherited from the fallback language.
...
**Additional Resource**: To learn how the localization logic works for published entries, refer to the [Retrieve Fallback Language Content for Published Entries](/docs/developers/multilingual-content/about-fallback-languages#retrieve-fallback-language-content-for-published-entries) section.
