...
Environment Variables are key-value pairs configured outside your source code so that each value can change depending on the [Environment](./environments.md).

Your source code can read these values to change behavior during the Build Step or Launch [Cloud Function](./cloud-functions.md) execution.
...
- When using Launch [Cloud Functions](./cloud-functions.md), ensure the combined size of all environment variables does not exceed **4 KB**.
...
You can add up to **128** environment variables per environment, with the total size of environment variables not exceeding **14 KB**. When using Launch [Cloud Functions](./cloud-functions.md), ensure the combined size of all environment variables does not exceed **4 KB**.
