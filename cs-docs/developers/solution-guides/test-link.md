---
title: "[Solution Guides Articles] -Test Link"
description: Solution guide article describing a CLI-based approach to automate generation of cross-stack content types with custom fields using the Content Management API (CMA).
url: https://www.contentstack.com/docs/developers/solution-guides/test-link
...
# [Solution Guides Articles] -Test Link

This page describes a solution guide approach for automating cross-stack content type generation with custom fields, intended for developers working with Contentstack stacks and extensions/apps, and useful when you need to synchronize or adapt content types across multiple stacks with minimal manual intervention.
...
It automates updating extension/app uids in a content type so the content type can be used across destination stacks where the corresponding extensions/apps have different uids.

### How does the script authenticate with Contentstack?
It attempts to use a stored authtoken from a local file (.authtoken) and falls back to email/password authentication using credentials provided in an .env file.

### Where does the script get the list of extensions and apps?
It fetches extensions and apps installed in a stack using an API endpoint, including: https://api.contentstack.io/v3/extensions?include_marketplace_extensions=true

### What is produced after running the script?
