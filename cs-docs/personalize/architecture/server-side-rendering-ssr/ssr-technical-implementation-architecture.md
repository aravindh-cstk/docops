...
![1 - SSR Technical Implementation Architecture.png](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/bltf9c10f30b5d1a1eb/676912ca91eb61a2a02fe1bf/1_-_SSR_Technical_Implementation_Architecture.png)

## Introduction

Contentstack Personalize offers flexibility and freedom in implementation, by being API-first and tech-stack agnostic. As long as you can make API calls to Contentstack, you can use [Personalize](/docs/personalize/about-personalize/). This document outlines the SSR architecture, focusing on how it handles personalized content delivery using both [Personalize Edge API](/docs/developers/apis/personalize-edge-api/) and [Contentstack Delivery API](/docs/developers/apis/content-delivery-api/).
...
1.  **Cache Granularity:** Ensure that cache keys are unique enough to distinguish between user variants but avoid excessive cache fragmentation.
2.  **Cache Expiration (TTL):** Set appropriate TTL values for personalized content. A TTL that's too long may serve outdated content, while one that's too short may decrease cache efficiency. Striking the right balance is crucial.
3.  **Cache Invalidation:** Implement cache invalidation logic so that when personalization rules or user data change, outdated variants are removed from the cache and don't get served.
4.  **Cache Size:** Be mindful of cache storage limits, especially with many unique variants, to avoid storage issues or degraded performance.
...
![3 - SSR Technical Implementation Architecture.png](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/blt9c34942e846e026b/676912ca558615e6076a3132/3_-_SSR_Technical_Implementation_Architecture.png)
...
![2 - SSR Technical Implementation Architecture.png](https://images.contentstack.io/v3/assets/blt2d43f51baca745a8/blt04a49b1f024b70cb/676912caf6d8ef8157c14258/2_-_SSR_Technical_Implementation_Architecture.png)
