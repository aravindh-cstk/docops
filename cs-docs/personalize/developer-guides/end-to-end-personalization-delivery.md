...
The below sequence diagram gives a better understanding of how request flows seamlessly between **Lytics**, **Personalize**, and **Contentstack CMS** to deliver personalized experiences efficiently.

![Sequence diagram of the personalization request flow between Lytics, Personalize, and Contentstack CMS](https://assets.contentstack.io/spaces/am51d76353d996c1fe/assets/am63b355df6912aed2/13150c5cfe6a9778ee05ec3e/Personalize-delivery-sequence.png?locale=en-us)
...
This collected data is sent to Lytics, which processes it to build audience segments.
...
To persist a user’s custom attributes at Personalize Edge, you can use the [Personalize Edge API](/docs/developers/apis/personalize-edge-api) or the [Edge SDK](/docs/developers/sdks/personalize-edge-sdk/javascript/about-javascript-personalize-edge-sdk).

To do it via the API, call the PATCH /user-attributes endpoint of the Edge API.

```
curl --location --request PATCH 
'https://<your_contentstack_region_edge_api_base_url>/user-attributes' \
--header 'x-project-uid: your_project_uid' \
--header 'x-cs-personalize-user-uid: you_visitors_user_uid' \
...
Alternatively, the more common and simpler approach is to use the set() method in the [Personalize Edge SDK](/docs/developers/sdks/personalize-edge-sdk/javascript/about-javascript-personalize-edge-sdk) to define key-value pairs representing user attributes. Ensure that the attribute keys you define match those created in the **Personalize Attributes** module.

```
import Personalize from '@contentstack/personalize-edge-sdk';

// if not using Contentstack's AWS NA region
Personalize.setEdgeApiUrl('<your_contentstack_region_edge_api_base_url>');
...
```
const activeVariants = personalizeSdk.getActiveVariants();
/**
Example active variants - experience short uid: variant short uid key-value pairs
{
  '0': 'a',
  '1': '2',
  '2': 'e',
...
curl -X GET 'https://personalize-edge.contentstack.com/manifest' \
  ...
  -H 'X-Cs-Personalize-User-Uid=<current users uid>' \
...
Use the active variants returned by the Manifest API to generate the variant aliases manually and fetch personalized entry data via the [Contentstack CDA](/docs/developers/apis/content-delivery-api) (REST, GraphQL, or SDK). Make sure to include the publish\_details field to access applied variant metadata.

Extract the variant alias from the Manifest response using the following logic:  
You should get an array that looks like \['cs\_personalize\_a\_0', 'cs\_personalize\_b\_1'\]

```
const variantAliases = manifestResponse.experiences
...
```
curl -X GET 'https://cdn.contentstack.io/v3/content_types/<content_type_uid>/entries?environment=<your environment>&locale=<your locale>&include_fallback=true&include_publish_details=true&include_rules=true&include_metadata=true&show_errors=true' \
  -H 'api_key: <stack_api_key>' \
  -H 'access_token: <delivery_token_of_the_stack>' \
  -H 'x-cs-variant-uid: <comma separated variant aliases string>' \
  -H 'Accept: application/json'
```

The variant aliases are passed as a comma-separated string. For example, the variant aliases \['cs\_personalize\_a\_0', 'cs\_personalize\_b\_1'\] should be passed as cs\_personalize\_a\_0, cs\_personalize\_b\_1 in the x-cs-variant-uid header.
...
```
{
  "entry": {
    ...
    "personalized_field_uid": "personalized content value",
    "publish_details": {
      "variants": {
        "<first_variant_uid>": {
...
Each key in the variant's object is a Variant UID. Each value includes an alias in the cs\_personalize\_<experience\_uid>\_<variant\_uid> format.
...
1.  **For specific experiences**
2.  **Dynamically for variants actually shown (recommended)**
...
personalizeSdk.triggerImpressions({ experienceShortUids });
```
...
A conversion represents a user action (for example, clicking a button or completing a purchase).

To record conversions, trigger an event using triggerEvent().
...
-   Variant aliases generated correctly (e.g., cs\_personalize\_a\_0).
...
-   Returned entries contain expected publish\_details.variants.
...
-   [Personalize Edge API: GET Manifest](/docs/developers/apis/personalize-edge-api/manifest#get-manifest)
-   [Content Delivery API: Entry Variants](/docs/developers/apis/content-delivery-api/entry-variants)
