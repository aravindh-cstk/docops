...
**Resolution:**
- Check the [**Launch Server Logs**](./edge-functions.md#server-logs) associated with the Edge Function’s execution to identify what caused the failure.
- Review the [**Edge Function’s**](./edge-functions.md) logic to ensure there are no unhandled exceptions.
...
**Resolution:**
- Check the **Launch Server Logs** or your configured [**Log Targets**](./log-targets.md).
...
Your server-side application returned response headers whose combined size was greater than the [allowed limit](./platform-limits-on-launch.md). This error returns a `502` response.
