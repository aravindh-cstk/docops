---
title: "[Contentstack Command-line Interface (CLI)] - Import Content Using the Seed Command | Beta Commands"
description: Import content into a Contentstack stack from GitHub repositories using the Contentstack CLI seed command (V2.x.x Beta).
url: https://www.contentstack.com/docs/developers/cli/import-content-using-the-seed-command/beta
...
# [Contentstack Command-line Interface (CLI)] - Import Content Using the Seed Command | Beta Commands

This page explains how to use the Contentstack CLI `seed` command (V2.x.x Beta) to import content into a stack from GitHub repositories. It is intended for developers and administrators who want to migrate or bootstrap stack content quickly, and should be used when importing from Contentstack’s GitHub organization or any public GitHub repository.

## Import Content Using the Seed Command | V2.x.x Beta
...
- **Stack preference**: You’ll be prompted to choose one of the following options.**Create a new stack**: Enter a name, and the CLI will begin the stack creation process.
...
|---|---|---|
| `--repo` | `-r` | GitHub organization name or GitHub user name/repository name. |
| `--org` | `-o` | Provide the organization UID to create a new stack. |
...
| `--fetch-limit` | `-l` | Limit for the number of organizations or stacks to be fetched. |
| `--yes` | `-y` | [Optional] Skip the stack confirmation. |
| `--alias` | `-a` | Alias of the management token. |
| `--locale` |  | -Master locale of the stack. |
...
Yes, but if the selected stack contains existing content, the CLI will prompt you to confirm before continuing, and you should ensure that it is empty.
...
Refer to the steps in the [Upload Stack’s Content on GitHub](#upload-stack-s-content-on-github) section.