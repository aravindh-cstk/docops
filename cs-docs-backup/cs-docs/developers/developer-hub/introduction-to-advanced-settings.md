...
**Additional Resource: **To learn more about the API call implementation, refer to the [API Integration in Developer Hub Apps](./api-integration-in-developer-hub-apps.md) documentation.
...
- [**Mappings**](#mappings)**: **Link a symbolic name to a path within the [server configuration](./app-config-location.md). This allows applications to reference stored values dynamically, enabling developers to securely access installation-specific sensitive data without exposing it in the frontend.
...
**Note: **Variables are app-specific, meaning all installations of the app share the same values. To store installation-specific or user-specific secret configurations, use [server configuration](./app-config-location.md) instead.
...
Each mapping refers to a value stored in [server configuration](./app-config-location.md) and can be used in the `appSdk.api` method with the syntax `{{map.MAPPING_NAME}}`. At runtime, this placeholder is replaced with the installation-specific value.
...
No. **Note: **Variables are app-specific, meaning all installations of the app share the same values. To store installation-specific or user-specific secret configurations, use [server configuration](./app-config-location.md) instead.
