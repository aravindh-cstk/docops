...
- By calling a [deploy hook](./deploy-hooks.md) generated on the environment.
- By clicking the **Redeploy** button on the deployment info page.

If you are redeploying from the deployment details page or using a [deploy hook](./deploy-hooks.md), you can use a `commitId` to redeploy, if you [imported the project using GitHub](./import-project-using-github.md).

When you click the **Redeploy** button, you can choose a commit to redeploy.

If you [imported the project by uploading a zip file](./import-project-using-file-upload.md), you can redeploy without a `commitId`. You can then choose from one of the following options to redeploy:
