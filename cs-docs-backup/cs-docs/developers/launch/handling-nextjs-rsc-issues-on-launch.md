...
This guide explains how to address issues with the **Next.js React Server Components (RSC)** when using [Next.js with App Router on Contentstack Launch](./nextjs-on-launch.md#app-router)—particularly when requests are routed through a CDN.
...
Fix the issue by using a [Launch Edge Function](./edge-functions.md) that removes the `Rsc: 1` header when the corresponding `_rsc` query parameter is missing.
