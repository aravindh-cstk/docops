...
**Note:** For [Edge URL Redirects](./edge-url-redirects.md), [Edge URL Rewrites](./edge-url-rewrites.md), and [Password Protection](./password-protection.md), consult their respective documentation. They provide a straightforward and declarative method to accomplish the same tasks.
...
You can [add environment variables](./environment-variables.md#add-environment-variables) by going to the corresponding environment’s Settings page on Launch.
...
- If you are using both `launch.json` and Launch Edge Functions in your project, then the [Edge URL Rewrites](./edge-url-rewrites.md) and [Edge URL Redirects](./edge-url-redirects.md) using `launch.json` will take precedence over the Edge Function. This means if you define a redirect for a route in both `launch.json` and Edge Function, then the redirect from `launch.json` will always be executed first.
- If you have enabled [password protection](./password-protection.md) for your website, then it will take precedence over the Edge Function.

**Note:** When you forward or rewrite a request to the origin server from the [Launch Edge URL Rewrites](./edge-url-rewrites.md) or the Launch Edge Function, the same request will not re-invoke the Launch Edge Function. Instead, it will be directly forwarded to the origin server through a cache layer.
...
- While setting up the project, by [Importing from a Git Repository](./import-project-using-github.md) or [Uploading a file](./import-project-using-file-upload.md), set the **Framework Preset** to `Other`.
...
**Note:** For both `Live` and `Archived` statuses, server logs are retained for **24** hours, with a maximum of **5000** most recent logs viewable. We recommend using [Log Targets](./log-targets.md) to forward server logs to your logging platform in real-time for future reference.
...
Contentstack Launch already offers basic [password protection](./password-protection.md) for your website using HTTP basic authentication. You can take security a step further by leveraging Contentstack Edge Functions to set up authentication based on [Contentstack account](https://www.contentstack.com/login/) logins.
